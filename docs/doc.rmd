...
* input_names : input names space separated. Names must match input files.

  > input_files = A.txt B.txt C.txt  
  > input_names = A B C

  or for paired (names can match paired input too.)
...
* input_flag_repeat: This argument to be used with 'input_multi=all'. If tool requires input flag for each input this command will add given flag before each input. If this argument doesn't exist spaces will be used between input files
...
> args = -i input_placeholder -o output_placeholder -name name_placeholder  
> named = True  
> suffix = job1_X  
> ext = txt  
...
> secondary_dump_dir = Y/secondary  
